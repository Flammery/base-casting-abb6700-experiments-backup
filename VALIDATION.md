...
2. 分别输入源 region 与 patch（例如 `12`、`1-2`），确认源 region 的每个最终 patch 独立生成设置页。
...
6. 旋转查看半透明灰色长方体，确认不会遮挡黄色、绿色和红色表面。
7. 点击“取消”不写文件；点击“应用”写同名 `*_avoidance.json`，不修改 `.rsp.json` 和分区 manifest。
8. 正式运行的 `summary.json` 记录 `avoidance_settings_path` 和每个区域的 `avoidance_volume` 范围/cell 数。
9. 不按朝上、朝下或法向正反过滤任何墙体；本轮不改变姿态库、IK/FK或碰撞判定。
