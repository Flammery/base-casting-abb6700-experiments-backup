...
把每个最终 planning label 的完整支撑面投影到 UV 平面，保留其三角化真实轮廓，
U/V 围绕轮廓中心独立扩大，再沿 N+/N- 拉伸。曲面同样先投影再拉伸。只有异形范围
内部或与边界相交的非支撑 cells 进入墙体网格，不区分朝上或朝下。
预览颜色为：黄色打磨面、绿色支撑面、红色范围内墙体、半透明灰色 UVN 范围；
范围外模型保持灰色。区域选择支持 source region 与 patch 混合输入，并支持中英文
逗号、顿号和分号。解析后自动显示；显示开关不修改参数，清除选择不删除已保存
sidecar。只有“应用”会写入弹窗底部所示的 `*_avoidance.json`。范围内墙体仍按当前
实验预算最多输出 `6000` 个三角形。
