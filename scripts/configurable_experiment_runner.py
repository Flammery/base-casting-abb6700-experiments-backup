...
import optimal_y_selection as selection  # noqa: E402
import window_conf_export as backend  # noqa: E402


FEED_VARIANTS = [("long_side", backend.RasterFeedDirection.LONG_SIDE)]
...
def module_label(value: float) -> str:
    return backend.module_coord_label(value)


def compact_number_label(value: float | int) -> str:
    """Format a numeric filename token without signs or decimal dots."""
...
def compact_coord_label(axis: str, spec: CoordinateSpec) -> str:
    if len(spec.values) == 1:
        return f"{axis}{compact_number_label(spec.values[0])}"
    step = spec.values[1] - spec.values[0]
    return (
        f"{axis}{compact_number_label(spec.values[0])},"
        f"{compact_number_label(step)},{compact_number_label(spec.values[-1])}"
    )


def compact_angles_label(angles: list[int]) -> str:
    normalized = [int(angle) % 360 for angle in angles]
    if len(normalized) == 1:
        return f"angle{compact_number_label(normalized[0])}"
    if len(normalized) >= 3:
        step = normalized[1] - normalized[0]
        if step > 0 and all(right - left == step for left, right in zip(normalized, normalized[1:])):
            return (
                f"angle{compact_number_label(normalized[0])},"
                f"{compact_number_label(step)},{compact_number_label(normalized[-1])}"
            )
    if len(normalized) <= 6:
        return "angle" + ",".join(compact_number_label(angle) for angle in normalized)
    return f"angle{compact_number_label(normalized[0])}_{compact_number_label(normalized[-1])}_n{len(normalized)}"
...
def planner_settings(project) -> backend.RasterPlannerSettings:
    """Build the shared raster settings without depending on a fixed run."""

    return backend.RasterPlannerSettings(
...
def selected_record_row(best_row: dict, selected_dir: Path) -> dict:
    """Copy one selected path into the stable result layout."""

    copied = selection.copy_optimal_files(best_row, selected_dir)
    row = dict(best_row)
...
def default_output_dir(
    ...
    """Return the next compact, human-readable result directory.

    Window, experiment, planner, and avoidance modes remain in ``summary.json``
    instead of lengthening the directory name.
    """

    del window_mode, experiment_mode, planner, avoidance_enabled
    date_suffix = date_suffix or datetime.now().strftime("%Y%m%d")
    results_root = results_root or backend.EXPERIMENT_DIR / "results"
    prefix = "-".join(
        (
            compact_coord_label("x", x_spec),
            compact_coord_label("y", y_spec),
            compact_coord_label("z", z_spec),
            compact_angles_label(angles),
        )
    )
    if run_number is None:
        ...
        if results_root.exists():
            for path in results_root.iterdir():
                ...
                if path.is_dir() and len(parts) == 3 and parts[-2] == date_suffix and parts[-1].isdigit():
    ...
    return results_root / f"{prefix}-{date_suffix}-{run_number:02d}"


def reserve_default_output_dir(
    x_spec: CoordinateSpec,
    y_spec: CoordinateSpec,
    z_spec: CoordinateSpec,
    angles: list[int],
    ...
    *,
    ...
    """Atomically reserve the next daily result number."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Configurable ABB6700 installation candidate runner.")
    parser.add_argument("--project", type=Path, default=backend.resolve_default_project_path())
    parser.add_argument(
        "--robot-config",
        type=Path,
    ...
    parser.add_argument("--boundary-margin", type=float, default=backend.BOUNDARY_MARGIN)
    ...
    parser.add_argument(
        ...
        type=float,
        default=backend.DEFAULT_MIN_CLEARANCE_MM,
...
def validate_args(args: argparse.Namespace, x_spec: CoordinateSpec, y_spec: CoordinateSpec, z_spec: CoordinateSpec) -> None:
    ...
    if float(getattr(args, "avoidance_min_clearance", backend.DEFAULT_MIN_CLEARANCE_MM)) < 0.0:
...
def bounds_from_args(args: argparse.Namespace) -> dict[str, tuple[float, float] | None]:
    if args.window_mode == "default":
        return {
            "x": tuple(backend.WINDOW_X),
            "y": tuple(backend.WINDOW_Y),
            "z": None,
        }
...
def configure_window(mode: str, bounds: dict[str, tuple[float, float] | None]) -> None:
    if bounds["x"] is not None:
        backend.WINDOW_X = bounds["x"]
    if bounds["y"] is not None:
        backend.WINDOW_Y = bounds["y"]
    ...
    backend.region_inside_window = region_inside_window
...
def enrich_candidate_row(model_x: float, model_y: float, model_z: float, angle: int, row: dict, path) -> dict:
    return {
        ...
        "score_max_abs_world_y": selection.score_max_abs_world_y(path),
        "score_max_abs_world_x": selection.score_max_abs_world_x(path),
        **row,
    }
...
def run_optimal_scan(
    ...
    backend.PROJECT_PATH = args.project
    backend.ANGLES = list(angles)
    backend.FEED_VARIANTS = list(FEED_VARIANTS)
    backend.BOUNDARY_MARGIN = float(args.boundary_margin)
    ...
    planning_regions = backend.manual_clip_regions(args.project, regions)
    ...
    if avoidance_settings_path is not None:
        avoidance_settings_payload = backend.load_avoidance_settings(avoidance_settings_path)
    ...
    resolved_avoidance_labels = [
        ...
        if backend.selector_matches(avoidance_selector_set, item["label"], item["source_region"])
    ]
    if project.polishing_tool.mass_kg <= 0.0:
        project.polishing_tool.mass_kg = backend.TOOL_LOAD_MASS_KG

    original_workpiece = project.workpiece.clone()
    importer = backend.CadImportService().import_model(project.workpiece.file_path)
    ...
    if robot_override is not None:
        ...
        collision_settings = backend.CollisionSettings(
            link_radius=backend.EXPERIMENT_LINK_RADIUS_MM,
            clearance=backend.EXPERIMENT_CLEARANCE_MM,
    ...
    vertices_by_region = {
        index: backend.clip_region_vertices(polydata, item["face_ids"], item.get("clip_polygon"), item.get("exclude_polygons"), item.get("raster_chart"))
    ...
    for model_x, model_y, model_z in iter_poses(x_spec, y_spec, z_spec):
        ...
        backend.MODEL_X = float(model_x)
        backend.MODEL_Y = float(model_y)
        backend.MODEL_Z = float(model_z)
        backend.OUTDIR = pose_dir

        for angle in angles:
            placement = backend.placement_for(
                original_workpiece,
            ...
            transform = backend.WorkpieceTransform(placement)
            inside_regions: list[int] = []

            for region_index, planning_region in enumerate(planning_regions, 1):
                if not backend.region_inside_window(vertices_by_region[region_index], transform):
                    continue
                ...
                for variant_name, feed_variant in FEED_VARIANTS:
                    ...
                    if args.planner == "auto":
                        path, use_hole_aware, planner_reason = backend.plan_region_uv_auto(*planner_arguments)
                    elif args.planner == "hole-aware":
                        use_hole_aware = True
                        planner_reason = "forced-cell-transfer"
                        path = backend.plan_region_uv_hole_aware(
                            *planner_arguments
                        )
                    else:
                        use_hole_aware = False
                        planner_reason = "forced-regular-raster"
                        path = backend.plan_region_uv(*planner_arguments)
                    avoidance_selected = backend.selector_matches(
                    ...
                    if path.waypoints and avoidance_selected:
                        ...
                        try:
                            seed_cell_ids = frozenset(backend.path_seed_cell_ids(path))
                            ...
                            if support_cache_key not in support_cache:
                                support_cache[support_cache_key] = (
                                    backend.support_for_planning_region(
                                        polydata,
                                        planning_region,
                                        seed_cell_ids,
                                    )
                            ...
                            if obstacle_key not in obstacle_cache:
                                volume = None
                                configured_record = (
                                    backend.avoidance_setting_for_label(
                                ...
                                if configured_record is not None:
                                    ...
                                    if frame_key not in volume_cell_bounds_cache:
                                        volume_cell_bounds_cache[frame_key] = (
                                            backend.avoidance_cell_bounds_uvn(
                                                polydata,
                                                volume_frame,
                                            )
                                        )
                                    ...
                                    obstacle_template = backend.build_obstacle_mesh_template(
                                        polydata,
                                        support,
                            ...
                            pose_selection = backend.select_robot_pose(
                                path,
                                polydata,
                                placement,
                    ...
                    if selected_path.waypoints:
                        row = backend.export_path_variant(
                            project,
                            placement,
                            selected_path,
                            angle,
                        ...
                        snapshot_path = (
                            pose_dir
                            / f"rz{angle:03d}"
                            / f"{backend.pose_file_label(model_x, model_y, model_z, angle)}.rsp.json"
                        )
                        if snapshot_path not in saved_snapshot_paths:
                            adjusted_project = backend.load_project_file(args.project)
                            if robot_override is not None:
                                robot_override.apply_to_project(adjusted_project)
                            adjusted_project.workpiece = placement.clone()
                            backend.save_project_file(snapshot_path, adjusted_project)
            ...
            coverage_rows.append(selection.coverage_table_row(model_x, model_y, model_z, angle, inside_regions))
    ...
    best_records = [selected_record_row(row, optimal_dir) for row in selection.choose_best_by_region(optimal_candidates)]
    candidate_table_rows = [selection.candidate_table_row(row) for row in candidate_rows]
    best_table_rows = [selection.best_table_row(row) for row in best_records]
    ...
    summary = {
        ...
        "avoidance_pose_roll_degrees": list(backend.POSE_ROLL_DEGREES),
        ...
        "window_base_xy": {"x": backend.WINDOW_X, "y": backend.WINDOW_Y},
        "window_shape": backend.WINDOW_SHAPE,
        "spacing_mm": backend.SPACING,
        "point_step_mm": backend.POINT_STEP,
        "boundary_margin_mm": backend.BOUNDARY_MARGIN,
        "orientation_mode": backend.ORIENTATION_MODE,
        "axis_mode": backend.AXIS_MODE,
        ...
        "tool_load_placeholder": backend.RAPID_LOAD_PLACEHOLDER,
        "confl_off": True,
...
def run_from_args(args: argparse.Namespace) -> Path:
    ...
    if args.output_dir is not None:
        ...
        outdir = reserve_default_output_dir(
            x_spec,
            y_spec,
            z_spec,
            angles,
